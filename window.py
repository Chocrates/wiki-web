...
class TreeWindow(Gtk.Window):
    def __init__(self, wikiNode):
        ...
        [
            self.node1GridList.append([child.title])
            for child in wikiNode.children
        ]
        ...
        self.urlGrid.attach_next_to(self.scrollable_treelist1,
                                    self.scrollable_treelist0,
        ...
        self.urlGrid.attach_next_to(self.scrollable_treelist2,
                                    self.scrollable_treelist1,
                                    Gtk.PositionType.RIGHT, 200, 10)

        self.renderWebKit(wikiNode.baseUrl)

        self.treeView0SignalId = self.treeView0.connect(
            'row-activated', self.node0Selection)
        self.treeView1SignalId = self.treeView1.connect(
            'row-activated', self.node1Selection)
        self.treeView2SignalId = self.treeView2.connect(
            'row-activated', self.node2Selection)
    ...
    def node0Selection(self, tree_view, path, column):
        try:
            ...
            if node.parent is None:
                ...
                self.node2GridList.clear()
                [
                    self.node2GridList.append([child.title])
                    for child in node.children
                ]
            else:
                ...
                self.node0GridList.clear()
                [
                    self.node0GridList.append([child.title])
